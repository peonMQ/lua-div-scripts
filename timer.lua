...
---@return _Timer @The timer instance.
function _Timer:new(expiration)
    local t = {}
    setmetatable(t, self)
    self.__index = self
    t.start_time = 0
...
function _Timer.current_time()
    return os.time()
end

---Reset the start time value to the current time.
---@param to_value number @The value to reset the timer to.
function _Timer:reset(to_value)
    self.start_time = to_value or _Timer.current_time()
end
...
function _Timer:timer_expired()
    if os.difftime(_Timer.current_time(), self.start_time) > self.expiration then
        return true
    else
        return false
    end
...
function _Timer:time_remaining()
    return self.expiration - os.difftime(_Timer.current_time(), self.start_time)
end
